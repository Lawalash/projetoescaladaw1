...
const uploadPlanilhas = multer({
  storage: criarStorage('uploads/planilhas'),
  limits: {
...
const uploadPontos = multer({
  storage: criarStorage('uploads/pontos'),
  limits: {
    fileSize: parseInt(process.env.MAX_FILE_SIZE, 10) || 20 * 1024 * 1024
  },
  fileFilter: (req, file, cb) => {
...
router.post('/inventario/upload', uploadPlanilhas.single('arquivo'), larController.uploadPlanilhaEstoque);
...
router.post('/pontos/importar', uploadPontos.single('arquivo'), larController.importarPontosColaboradores);
